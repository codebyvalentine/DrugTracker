  ...
  Future<void> _fetchMedications() async {
    ...
    try {
      ...
      for (var doc in medicationsSnapshot.docs) {
        ...
        if (_selectedDate.isBefore(startDate) || _selectedDate.isAfter(endDate)) {
        ...
        for (var reminderDoc in remindersSnapshot.docs) {
          ...
          if (reminderData['taken'] == true) {
            print("Skipping taken reminder for ${medicationData['drugName']} at ${reminderData['time']}.");
            continue;
          }
          ...
          if (frequency == 'Daily') {
            ...
            final int intervalDays = int.parse(frequency.split(' ')[1]); // Get the interval (e.g., 2 days)
            final int daysSinceStart = _selectedDate.difference(startDate).inDays;
          ...
          if (shouldAddReminder) {
            ...
            reminder['name'] == drugName &&
              ...
              print("Duplicate reminder for $drugName at $scheduledTime skipped.");
            }
          }
        }
      }
  ...
  void _onTakeMedication(Map<String, dynamic> medication) async {
    ...
    try {
      ...
      if (medicationQuery.docs.isNotEmpty) {
        ...
        final remindersCollection = medicationDoc.reference.collection('reminders');
        ...
        if (reminderQuery.docs.isNotEmpty) {
          ...
          print("Reminder for ${medication['name']} at ${medication['time']} marked as taken in Firestore.");
        } else {
          print("No matching reminder found in Firestore for ${medication['name']} at ${medication['time']}.");
        }
      } else {
        print("No matching medication found in Firestore for ${medication['name']}.");
      }
